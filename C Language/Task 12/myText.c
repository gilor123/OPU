...
Name: myText.c
Author: Gil Or
Description:  The program gets from the user a selected data structure - dynamic buffer or linked list.
              Than the program prints the text in accordance with the requirements in MAMAN 12
*/
...
/* The function reads the text from the user and store it in the selected data structure*/ 
...
/* The function prints the text in accordance with MAMAN12 requirements*/ 
...
void freeList (pItem headOfList) {
    if ((headOfList->next) == NULL) {
        return;
    }
    freeList(headOfList->next);
    free (headOfList);
}
